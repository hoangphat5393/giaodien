...
document.addEventListener("DOMContentLoaded", function () {
 ...
 //  const items = container.querySelectorAll('.detail_title');
 ...
 //  });
  ...
  //   });
    ...
    const menuItems = document.querySelectorAll('.ttl');

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Get the ID of the visible section (right side)
            const sectionId = entry.target.id;

            // Remove active from all menu items
            menuItems.forEach((item) => item.classList.remove('active'));

            // Add active to the menu item with matching ID (menu-[id])
      ...
      {
        threshold: 1, // activate when 60% of section is visible
      }
    );

    // Observe all right-side sections
    sections.forEach((section) => observer.observe(section));

    // Optional: Click menu scrolls to matching section
    menuItems.forEach((item) => {
      item.addEventListener('click', () => {
        const targetId = item.id.replace('menu-', ''); // extract 'a', 'b', etc.
